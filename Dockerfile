...
# Low-resource build: cap Go compile parallelism (host has 4 cores / 3.6G).
# Override with --build-arg GO_MAXPROCS=N.
ARG GO_MAXPROCS=2
ENV GOMAXPROCS=${GO_MAXPROCS}
# Atlas brand build: outbound proxy.golang.org is unreachable from this
# build host; use a domestic Go module proxy instead.
